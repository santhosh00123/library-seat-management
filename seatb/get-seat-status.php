...
try {
    ...
    // FIXED: Updated query to handle exact time slot boundaries properly
    // A seat is conflicted only if the time ranges actually OVERLAP
    // Consecutive bookings (9-10 AM, then 10-11 AM) should NOT conflict
    $sql = "SELECT seat_id, status, start_time, duration, student_id, booking_code, attendance_code
        FROM seat_bookings 
        WHERE booking_date = :date 
        AND status IN ('booked', 'attended')
        AND (
            -- True overlap: one booking starts before the other ends
            -- AND the first booking ends after the other starts
